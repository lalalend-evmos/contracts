...
import "./MIAVaultProxy.sol";
import "./MIAVaultStorage.sol";
import "./MIAVaultErrorReporter.sol";
...
contract MIAVault is MIAVaultStorage, ECDSA {
    ...
    event StoreUpdated(address oldMIA, address oldStore, address newMIA, address newStore);
    ...
    function add(
        ...
        onlyAdmin
    {
        require(address(MIAStore) != address(0), "Store contract addres is empty");
        ...
        IMIAStore(MIAStore).setRewardToken(_rewardToken, true);
    ...
    /**
     * @notice Deposit MIAVault for MIA allocation
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            IMIAStore(MIAStore).safeRewardTransfer(_rewardToken, msg.sender, pending);
        }
        ...
        if (address(pool.token) == address(MIAAddress)) {
    ...
    /**
     * @notice Execute withdrawal to MIAVault for MIA allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to MIAVault for MIA allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
        ...
        nonReentrant
    {
        ...
        // Update Delegate Amount
        if (_rewardToken == address(MIAAddress)) {
    ...
    // View function to see pending MIAs on frontend.
    ...
    /**
     * @notice Get the MIA stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(MIAAddress != address(0), "MIAVault::getStakeAmount: MIA address is not set");

        PoolInfo[] storage poolInfo = poolInfos[MIAAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(MIAAddress)) {
                UserInfo storage user = userInfos[MIAAddress][pid][account];
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s)
        external
    {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("MIAVault")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "MIAVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MIAVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "MIAVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "MIAVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "MIAVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the MIA stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "MIAVault::getPriorVotes: not yet determined");
    ...
    function _become(MIAVaultProxy miaVaultProxy) external {
        require(msg.sender == miaVaultProxy.admin(), "only proxy admin can change brains");
        require(miaVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setMIAStore(address _MIA, address _MIAStore) external onlyAdmin {
        address oldMIAContract = MIAAddress;
        address oldStore = MIAStore;
        MIAAddress = _MIA;
        MIAStore = _MIAStore;

        _notEntered = true;

        emit StoreUpdated(oldMIAContract, oldStore, _MIA, _MIAStore);
    }
}