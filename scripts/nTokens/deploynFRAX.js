...
async function main() {
  ...
  const underlying = "0x9f3A6b116efFc9736ff2dDeF39C160e6854fD59c";
  ...
  const nameOfToken = "Lalalend FRAX";
  const symbol = "nFRAX";
  const decimals = 8;
  ...
  const NFRAX = await ethers.getContractFactory("NErc20Delegator");
  const nFrax = await NFRAX.deploy(
    underlying, 
    comptroller,
    interestRateModel,
    initialExchangeRate,
  ...
  await nFrax.deployed();

  console.log("Deploying contracts with the account:", deployer.address);
  console.log("Deployed contract NFRAX at address:", nFrax.address);
