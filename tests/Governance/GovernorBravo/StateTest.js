const {
  ...
  describe('GovernorBravo#state/1', () => {
    let mia, miaVault, gov, root, acct, guardian, delay, timelock;
    ...
    beforeAll(async () => {
      ...
      delay = evmosUnsigned(2 * 24 * 60 * 60).mul(2)
    ...
    it("Canceled", async () => {
      ...
      await send(miaVault, 'delegate', [accounts[0]], { from: accounts[0] });
      await mineBlock()
      ...
      await send(miaVault, 'delegate', [root], { from: accounts[0] });
