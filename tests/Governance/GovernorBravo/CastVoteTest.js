const {
  ...
  describe("governorBravo#castVote/2", () => {
    let gov, root, a1, guardian, accounts, govDelegate, miaVault, mia;
    ...
    beforeAll(async () => {
      ...
      await send(miaVault, 'delegate', [root]);
  
      govDelegate = await deploy('GovernorBravoDelegateHarness');
      gov = await deploy(
        'GovernorBravoDelegator',
        [address(0), miaVault._address, root, govDelegate._address, 86400, 1, evmosMantissa(1e4), guardian]
      );
    ...
    describe("We must revert if:", () => {
      it("We cannot propose without enough voting power by depositing mia to the vault", async () => {
        await expect(
      ...
      describe("after we deposit mia to the vault", () => {
        beforeAll(async () => {
    ...
    describe("Otherwise", () => {
      ...
      describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
        ...
        it("and we add that ForVotes", async () => {
          ...
          expect(new BigNumber(afterFors)).toEqual(new BigNumber(beforeFors).plus(evmosMantissa(400001)));
        })
        ...
        it("or AgainstVotes corresponding to the caller's support flag.", async () => {
          ...
          expect(new BigNumber(afterAgainsts)).toEqual(new BigNumber(beforeAgainsts).plus(evmosMantissa(400001)));
        });
      });
  
      describe('castVoteBySig', () => {
        const Domain = (gov) => ({
          name: 'Nemo Governor Bravo',
