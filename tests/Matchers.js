...
const { address, evmosUnsigned } = require('./Utils/EVMOS');
const diff = require('jest-diff').default;
const { ComptrollerErr, SEBControllerErr, TokenErr, IRErr, MathErr } = require('./Errors');
...
expect.extend({
  ...
  toHaveSEBTrollError(actual, expectedErrorName) {
    return hasError.call(this, actual, expectedErrorName, SEBControllerErr);
  },
  ...
  toHaveSEBTrollFailure(result, err, info, detail=undefined) {
    return hasFailure.call(this, result, err, info, detail, SEBControllerErr, 'toHaveSEBTrollFailure');
  },
  ...
  toHaveSEBTrollMathFailure(result, info, detail) {
    return hasFailure.call(this, result, 'MATH_ERROR', info, detail && (MathErr.Error[detail] || -1), SEBControllerErr, 'toHaveSEBTrollMathFailure');
  },
  ...
  toHaveSEBTrollReject(result, info, detail) {
    return hasFailure.call(this, result, 'REJECTION', info, detail && ComptrollerErr.Error[detail], SEBControllerErr, 'toHaveSEBTrollReject');
  },

  toHaveSEBTrollErrorTuple(result, tuple, cmp=undefined) {
    return hasErrorTuple.call(this, result, tuple, SEBControllerErr, cmp);
  },

  toEqualNumber(actual, expected) {
    return match.call(this, actual, expected, evmosUnsigned(actual).eq(evmosUnsigned(expected)), opts('toEqualNumber'));
  },
