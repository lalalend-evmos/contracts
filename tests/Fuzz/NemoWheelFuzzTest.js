...
describe.skip('NemoWheelFuzzTest', () => {
  // This whole test is fake, but we're testing to see if our equations match reality.

  // First, we're going to build a simple simulator of the Nemo protocol
  ...
  let initialState = globals => {
    return {
      ...
      miaSupplySpeed: new bn(1),
      miaSupplyIndex: new bn(1),
      miaSupplyIndexSnapshots: {},
      miaSupplyIndexUpdatedBlock: globals.blockNumber,

      miaBorrowSpeed: new bn(1),
      miaBorrowIndex: new bn(1),
      miaBorrowIndexSnapshots: {},
      miaSupplyIndexUpdatedBlock: globals.blockNumber,

      miaAccruedWithCrank: {}, // sebve method, accruing all accounts every block
      miaAccruedWithIndex: {}, // with indices

      activeBorrowBlocks: new bn(0), // # blocks with an active borrow, for which we expect to see mia distributed. just for fuzz testing.
  ...
  let trueUpNemo = (globals, state) => {
    state = accrueInterest(globals, state);

    state = Object.keys(state.miaSupplyIndexSnapshots).reduce(
      (acc, account) => supplierFlywheelByIndex(globals, state, account),
      state
    );

    state = Object.keys(state.miaBorrowIndexSnapshots).reduce(
  ...
  // manual flywheel loops through every account and updates mia accrued mapping
  ...
  let flywheelByCrank = (
    ...
    for (let [account, balance] of Object.entries(balances)) {
      if (isPositive(totalSupply)) {
        miaAccruedWithCrank[account] = get(
          state.miaAccruedWithCrank[account]
        ).plus(
          deltaBlocks
            .times(miaSupplySpeed)
            .times(balance)
            .div(totalSupply)
        );
      }
    ...
    return {
      ...state,
      miaAccruedWithCrank: miaAccruedWithCrank,
    };
  };

  // real deal mia index flywheel™️
  let borrowerFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      totalBorrows,
      borrowBalances,
      miaBorrowIndexUpdatedBlock,
      borrowIndex,
      borrowIndexSnapshots
    } = state;

    let deltaBlocks = globals.blockNumber.minus(miaBorrowIndexUpdatedBlock);
    if (isPositive(totalBorrows)) {
      let scaledTotalBorrows = totalBorrows.div(borrowIndex);
      miaBorrowIndex = miaBorrowIndex.plus(
        miaBorrowSpeed.times(deltaBlocks).div(scaledTotalBorrows)
      );
    }

    let indexSnapshot = miaBorrowIndexSnapshots[account];

    if (
      indexSnapshot !== undefined &&
      miaBorrowIndex.isGreaterThan(indexSnapshot) &&
      ...
      miaAccruedWithIndex[account] = get(miaAccruedWithIndex[account]).plus(
        borrowBalanceNew
          .div(borrowIndex)
          .times(miaBorrowIndex.minus(indexSnapshot))
      );
    }

    return {
      ...state,
      miaBorrowIndexUpdatedBlock: globals.blockNumber,
      miaBorrowIndex: miaBorrowIndex,
      miaBorrowIndexSnapshots: {
        ...state.miaBorrowIndexSnapshots,
        [account]: miaBorrowIndex
      }
    };
  };
  ...
  let supplierFlywheelByIndex = (globals, state, account) => {
    let {
      ...
      miaAccruedWithIndex,
      totalSupply,
      miaSupplyIndexUpdatedBlock
    } = state;

    let deltaBlocks = globals.blockNumber.minus(miaSupplyIndexUpdatedBlock);

    if (isPositive(totalSupply)) {
      miaSupplyIndex = miaSupplyIndex.plus(
        miaSupplySpeed.times(deltaBlocks).div(totalSupply)
      );
    }

    let indexSnapshot = miaSupplyIndexSnapshots[account];
    if (indexSnapshot !== undefined) {
      // if had prev snapshot,  accrue some mia
      miaAccruedWithIndex[account] = get(miaAccruedWithIndex[account]).plus(
        balances[account].times(miaSupplyIndex.minus(indexSnapshot))
      );
    }
    ...
    return {
      ...
      miaSupplyIndexSnapshots: {
        ...state.miaSupplyIndexSnapshots,
        [account]: miaSupplyIndex
      },
      miaAccruedWithIndex: miaAccruedWithIndex
    };
  };
  ...
  // assert amount distributed by the crank is expected, that it equals # blocks with a supply * mia speed
  let crankCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.activeSupplyBlocks
      .times(state.miaSupplySpeed)
      .plus(state.activeBorrowBlocks.times(state.miaBorrowSpeed));

    let actual = Object.values(state.miaAccruedWithCrank).reduce(
    ...
    invariantFn(
      ...
      expected,
      actual,
      `crank method distributed mia inaccurately`
    );
  };

  // assert mia distributed by index is the same as amount distributed by crank
  let indexCorrectnessInvariant = (globals, state, events, invariantFn) => {
    let expected = state.miaAccruedWithCrank;
    let actual = state.miaAccruedWithIndex;
    invariantFn(
      (expected, actual) => {
  ...
  let runEvents = (globals, initState, events) => {
    ...
    return trueUpNemo(globals, state);
  };
