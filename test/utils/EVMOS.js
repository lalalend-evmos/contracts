...
async function evmosBalance(addr) {
  return new BigNum(await web3.eth.getBalance(addr));
}

async function sebBalance(seb, addr) {
  return new BigNum(await web3.eth.getBalance(addr));
}

async function evmosGasCost(receipt) {
...
function evmosExp(num) { return evmosMantissa(num, 1e18) }
function evmosDouble(num) { return evmosMantissa(num, 1e36) }
