...
const {
  ...
  describe('NToken', function () {
    let root, minter, redeemer, payer, receiver;
    let nToken, NDelegator, NDelegatee, mia, MIA, nDelegatee, nDelegator;
    beforeEach(async () => {
      [owner, minter, redeemer, receiver, addrGuardian, addrTreasury] = await ethers.getSigners();
      payer = minter;
      //constructs a NToken 
     ...
     // NDelegatee = await ethers.getContractFactory("NErc20DelegateHarness");
       ...
       const NDelegatee = await ethers.getContractFactory("NErc20Harness");
       ...
       await comptroller_.setTreasuryData(addrGuardian.address, addrTreasury.address, 1e14);
      ...
      await comptroller_._supportMarket(nToken.address)
    ...
    describe('mint', () => {
      ...
      it.only("emits a mint failure if interest accrual fails", async () => {
        await nToken.interestRateModel.setFailBorrowRate(true);
        await expect(quickMint(nToken, minter, mintAmount)).to.be.revertedWith("revert INTEREST_RATE_MODEL_ERROR");
      });
